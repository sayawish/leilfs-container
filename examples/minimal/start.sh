...
ensure_cluster_network() {
  if docker network inspect "${CLUSTER_NETWORK_NAME}" >/dev/null 2>&1; then
    return 0
  fi

  docker network create \
    -d ipvlan \
    --subnet "${LEILFS_SUBNET}" \
    --gateway "${LEILFS_GATEWAY}" \
    --aux-address "sfsmaster=${FLOATING_IP}" \
    -o parent="${PARENT_INTERFACE}" \
    -o ipvlan_mode=l2 \
    "${CLUSTER_NETWORK_NAME}" >/dev/null
}
...
export LEILFS_SUBNET="${LEILFS_SUBNET:-172.31.0.0/24}"
export LEILFS_GATEWAY="${LEILFS_GATEWAY:-172.31.0.1}"
export FLOATING_IP="${FLOATING_IP:-172.31.0.10}"
export MASTER1_IP="${MASTER1_IP:-172.31.0.11}"
export MASTER_HOST="${MASTER_HOST:-sfsmaster}"
export SAUNAFS_HDD_COUNT="${SAUNAFS_HDD_COUNT:-1}"
export CGI_EXTERNAL_PORT_BASE="${CGI_EXTERNAL_PORT_BASE:-29425}"
export CLUSTER_NETWORK_NAME="${CLUSTER_NETWORK_NAME:-leilfs-minimal-net}"
export PARENT_INTERFACE="${PARENT_INTERFACE:-$(default_parent_interface)}"

PROJECT_NAME="${COMPOSE_PROJECT_NAME:-leilfs-minimal}"
CHUNKSERVER_SCALE="${CHUNKSERVER_SCALE:-1}"
CLIENT_SCALE="${CLIENT_SCALE:-1}"
METALOGGER_SCALE="${METALOGGER_SCALE:-1}"
...
if [[ -z "${PARENT_INTERFACE}" ]]; then
  echo "Unable to determine parent interface automatically. Set PARENT_INTERFACE explicitly." >&2
  exit 1
fi
